...
def get_jetparts(tree, jet):
    ...
    begin = jet.reco_jet.particles_begin
    end = jet.reco_jet.particles_end
...
for event in range(n_tot):
    tree.GetEntry(event)
    jetmatches = jm.JetMatcher(tree, jetfilter='all')
    jets = jetmatches.get_reco_jets()
